	 ...
	 public boolean insertEmployee(Employee emp)
	  {
		  ...
		  try
		  {
			  ...
			  pst=con.prepareStatement("insert into employ values(?,?,?,?)");
		  ...
		  finally
		  {
		    ...
		    public boolean modifyEmployee(Employee emp)
		    {
		  	  ...
		  	  try
		  	  {
		  		  Class.forName("oracle.jdbc.driver.OracleDriver");
		  		  con = DriverManager.getConnection(url,"scott","tiger");
		  		  pst=con.prepareStatement("update employ set ename=?,salary=? where empid=?");
		  	   ...
		  	   public boolean removeEmployee(int  empid)
			    {
			  	  ...
			  	  try
			  	  {
			  		  Class.forName("oracle.jdbc.driver.OracleDriver");
			  		  con = DriverManager.getConnection(url,"scott","tiger");
			  		  pst=con.prepareStatement("delete from employ where id=?");
			  		 pst.setInt(1,empid);
			  		  count = pst.executeUpdate();
			  	  }
			  	catch(Exception ex)
			   ...
			   return count ==1;
			  		...
			  		public Employee getEmployee(int empid)
				    {
				  	  ...
				  	  try
				  	  {
				  		  Class.forName("oracle.jdbc.driver.OracleDriver");
				  		  con = DriverManager.getConnection(url,"scott","tiger");
				  		  pst=con.prepareStatement("select * from employ where id=?");
				  		  pst.setInt(1, empid);
				  		  rs = pst.executeQuery();
				  		  if(rs.next())
